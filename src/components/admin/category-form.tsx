...
export function CategoryForm({ category, parentCategories }: CategoryFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const data = {
      name: form.name,
      slug: category?.slug || slugify(form.name),
      ...
      <CardContent className="pt-6">
        <form onSubmit={handleSubmit} className="space-y-4 max-w-lg">
          ...
          <div>
            ...
            <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="mt-1 flex w-full rounded-md border border-input bg-background px-3 py-2 text-sm min-h-[100px]" />
          </div>
          <div>
